...
fun CaloriesChartScreen(
    ...
    Scaffold(
        topBar = {
            TopAppBar(
                title = { Text(stringResource(R.string.calories_screen)) },
                navigationIcon = {
                    IconButton(onClick = {
        ...
        content = { innerPadding ->
            ...
            LaunchedEffect(uiState) {
                ...
                if (uiState is CaloriesUiState.Uninitialized) {
                    onPermissionsResult()
                }
                ...
                if (uiState is CaloriesUiState.Error && errorId.value != uiState.uuid) {
//                    onError(uiState.exception) todo
                    errorId.value = uiState.uuid
                }
            }

            if (viewModel.uiState != CaloriesUiState.Uninitialized) {
                LazyColumn(modifier = Modifier.padding(innerPadding)) {
                    if (!viewModel.permissionsGranted) {
                        ...
                        item {
                            ActivityChart(viewModel.caloriesList, onItemClick = { index ->
                        ...
                        if (viewModel.isShowingGoalEdit) {
                            item {
                                Column(modifier = Modifier.padding(all = 16.dp)) {
                                    ...
                                    GoalSlider(
                                        initialValue = viewModel.caloriesGoal.toFloat(),
                                        goalText = { value ->
                                            "${value.toInt()} калорий"
                                        },
                                        maxValue = 5000f,
